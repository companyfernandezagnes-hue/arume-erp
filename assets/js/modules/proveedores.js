...
export async function render(container, supabase, db, opts = {}) {
    ...
    db.proveedores.forEach(p => {
        ...
        // Guardamos TODOS los datos del proveedor en el objeto stats
        stats[key] = { 
            ...p, // Heredamos todo (id, n, cif, iban, bic, tel, mail, fam...)
            total: 0, 
            orders: 0,
            isSaved: true 
        };
        ...
        if (checkPeriod(a.date)) {
            ...
            if (!stats[key]) {
                stats[key] = { 
                    ...
                    n: a.prov, 
                    tel: '', mail: '', fam: 'General', nif: '', iban: '', bic: '',
                    total: 0, orders: 0, isSaved: false 
                };
            }
            stats[key].total += (parseFloat(a.total) || 0);
            stats[key].orders += 1;
        }
    });
    ...
    <div class="animate-fade-in space-y-6 pb-24">
        ...
        <header class="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 gap-4">
            <div>
                ...
                <p class="text-[10px] text-indigo-500 font-bold uppercase tracking-widest">Volumen de Compra: ${fmt(totalCompras)}</p>
            </div>
        ...
        <div class="grid grid-cols-3 gap-2 md:gap-4 mb-2">
            ${ranking.slice(0,3).map((p, i) => {
                ...
                if(p.total === 0) return ''; 
                return `
                <div class="p-3 rounded-2xl border ${colors[i]} text-center shadow-sm flex flex-col justify-center items-center relative overflow-hidden cursor-pointer hover:shadow-md transition" onclick="window.editarProv('${p.id || ''}', '${p.n.replace(/'/g, "\\'")}')">
                    ...
                    <p class="font-mono font-bold text-xs">${fmt(p.total)}</p>
                </div>`;
            }).join('')}
        </div>
        ` : ''}
    ...
    const pintarLista = () => {
        ...
        lista.innerHTML = filtered.map((p, i) => {
            ...
            const badgeIBAN = hasIBAN 
                ? '<span class="text-[8px] text-emerald-500 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100 font-bold">IBAN OK</span>' 
            ...
            <div class="bg-white p-4 rounded-2xl border border-slate-100 flex justify-between items-center group hover:border-indigo-300 transition cursor-pointer shadow-sm relative overflow-hidden" onclick="window.editarProv('${p.id || ''}', '${p.n.replace(/'/g, "\\'")}')">
                ...
                <div class="flex items-center gap-3 relative z-10">
                    ...
                    <div>
                        ...
                        <div class="flex gap-2 mt-1 items-center">
                            <span class="text-[9px] font-bold text-slate-500 uppercase tracking-wider">${p.fam || 'General'}</span>
                            ${badgeIBAN}
                        </div>
                    </div>
                </div>
                ...
                <div class="text-right relative z-10">
                    <div class="font-black text-slate-700 text-sm">${fmt(p.total)}</div>
                    <div class="text-[9px] text-slate-400 font-bold">${p.orders} pedidos</div>
                </div>

                <div class="absolute right-20 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity bg-white pl-4">
    ...
    window.editarProv = (id = null, tempName = '') => {
        // Objeto base
        let p = { id: Date.now().toString(), n: tempName, fam: 'General', tel: '', mail: '', cif: '', iban: '', bic: '' };
        
        // Si hay ID, buscamos en la base de datos REAL
        if (id) {
        ...
        modal.innerHTML = `
            <div class="bg-white w-full max-w-lg rounded-[2.5rem] p-8 shadow-2xl animate-slide-up relative overflow-y-auto max-h-[90vh]">
                ...
                <h3 class="text-xl font-black text-slate-800 mb-1">${id ? 'Editar Ficha' : 'Alta Proveedor'}</h3>
        ...
        modal.querySelector("#btnSaveProv").onclick = async () => {
            const nuevo = {
                ...
                cif: modal.querySelector("#p-cif").value.toUpperCase(), // CIF siempre existe
