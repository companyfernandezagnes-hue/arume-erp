/* =============================================================
   🏢 MÓDULO: GASTOS FIJOS (Estructura de Costes & Mochila)
   ============================================================= */
...
export async function render(container, supabase, db, opts = {}) {
    ...
    // --- CEREBRO DE CÁLCULO (Prorrateo Real) ---
    const getMensual = (g) => {
        let amount = parseFloat(g.amount) || 0;
        // Si el gasto no está activo, no cuenta
        ...
        if (g.freq === 'semanal') return amount * 4.33; // Promedio semanas/mes
    ...
    const pintarLista = () => {
        ...
        }).sort((a,b) => getMensual(b) - getMensual(a));
        ...
        lista.innerHTML = filtered.map(g => {
            ...
            <div class="bg-white p-5 rounded-[2rem] border ${isPaid ? 'border-emerald-200 bg-emerald-50/30' : 'border-slate-100'} shadow-sm relative group hover:shadow-md transition">
                ...
                <div class="flex justify-between items-end mt-4 pt-3 border-t border-slate-50">
                    ...
                    <div class="text-right">
                        <p class="text-[9px] text-indigo-400 font-bold uppercase">Impacto Mensual</p>
    ...
    window.togglePago = async (id) => {
        ...
        if (idx === -1) {
            pagados.push(id);
            await saveFn("Marcado como pagado ✅");
        } else {
            pagados.splice(idx, 1);
            await saveFn("Desmarcado");
        }
        pintarLista();
    };

    // 5. EDICIÓN Y CREACIÓN (MODAL COMPLETO)
    window.editarGasto = (id = null) => {
        ...
        modal.innerHTML = `
            <div class="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl animate-slide-up relative">
                ...
                <h3 class="text-xl font-black text-slate-800 mb-6">${id ? 'Editar Gasto' : 'Nuevo Gasto Fijo'}</h3>

                <div class="space-y-4">
                    <div>
                        <label class="text-[9px] font-bold text-slate-400 uppercase ml-2">Nombre del Gasto</label>
                        <input id="g-name" type="text" value="${g.name}" placeholder="Ej. Alquiler, Gestoría..." class="w-full p-3 bg-slate-50 rounded-xl font-bold text-sm border border-slate-100 outline-none focus:ring-2 focus:ring-indigo-500 transition">
                    </div>

                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="text-[9px] font-bold text-slate-400 uppercase ml-2">Importe Total (€)</label>
                    ...
                    <div>
                        <label class="text-[9px] font-bold text-slate-400 uppercase ml-2">Categoría (Para Informes)</label>
                    ...
                    <div class="pt-4 flex flex-col gap-3">
                        ...
                        ${id ? `<button id="btnArchiveGasto" class="w-full text-rose-400 text-[10px] font-bold uppercase tracking-widest hover:text-rose-600">Eliminar / Archivar</button>` : ''}
                    </div>
                </div>
            </div>
        `;
